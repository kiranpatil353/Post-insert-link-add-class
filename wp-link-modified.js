...
( function( $ ) {
	...
	wpLink = {
		...
		open: function( editorId ) {
			...
			if($("input[id*='wp-link-class']").length < 1 ){
				$('#wp-link-submit').val('Add Link');
                 $('#link-options').append('<div><label><span>CSS Class</span><input id="wp-link-class" name="wp-link-class" type="text"></label></div>' ) 
			}
		...
		htmlUpdate: function() {
			...
			var csclass = " class="+"'"+$('#wp-link-class').val()+"'";
			html = html.slice(0, -1);
			html = html + csclass + ">";
